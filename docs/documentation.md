...
The app reads bundled Bible and featured-passage data through domain services. Guest saved passages use browser storage, while authenticated saved passages, practice history, and reflections use Supabase behind domain store contracts.

The data model, trust boundaries, storage strategy, Row Level Security rules, and future translation considerations are documented in [`data-and-security.md`](data-and-security.md). Exact cloud schema behaviour remains defined in [`supabase/schema.sql`](../supabase/schema.sql).
...
Product direction, current limitations, and delivery priorities are tracked in [`product-status.md`](product-status.md).
